...
The switch uses local `signed_gadget_invert_alloc`, not upstream
`gadget_invert_alloc`: lower digits are balanced and the final quotient is
retained exactly modulo `q`. This keeps the upstream gadget vector and matrix
width while avoiding a dropped top carry.
