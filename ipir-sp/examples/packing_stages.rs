...
fn main() {
    ...
    for sample in 0..samples + 5 {
        ...
        if mode >= 2 {
            ...
            let (name, value) = match mode {
                ...
                4 => (
                    "read",
                    pre.par_iter()
                        .map(|p| p.matrix_read_checksum())
                        .reduce(|| 0, |a, b| a ^ b),
                ),
                _ => panic!("invalid benchmark mode"),
            };
            ...
            if sample >= 5 {
                println!(
                    "{}",
                    serde_json::json!({"kind":name,"sample":sample-5,"blocks":blocks,"ell":ell,"ms":ms,"threads":rayon::current_num_threads()})
                );
            }
            continue;
        }
        let start = Instant::now();
        let (result, key_ms, contribution_ms, finish_ms) = if cached {
            let prepared = pre[0].prepare_keys(&keys).unwrap();
            let key_ms = start.elapsed().as_secs_f64() * 1000.;
            let stage = Instant::now();
            let pending: Vec<_> = pre
                .par_iter()
                .map(|pre| pre.prepare_pack(&prepared).unwrap())
                .collect();
            let contribution_ms = stage.elapsed().as_secs_f64() * 1000.;
            let stage = Instant::now();
            let result: Vec<_> = pending
                .into_iter()
                .zip(&bodies)
                .map(|(pending, b)| pending.finish(black_box(b)).unwrap())
                .collect();
            (
                result,
                key_ms,
        ...
        if cached && sample == 0 {
            for ((block, b), ct) in pre.iter().zip(&bodies).zip(&result) {
                assert_eq!(block.pack(b, &keys).unwrap().rows(), ct.rows());
            }
        }
